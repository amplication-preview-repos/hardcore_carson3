...
const service = {
  ...
  csvExports: () => FIND_MANY_RESULT,
  csvExport: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("CsvExport", () => {
  ...
  test("GET /csvExports/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/csvExports"}/${nonExistingId}`)
  ...
  test("GET /csvExports/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/csvExports"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /csvExports existing resource", async () => {
